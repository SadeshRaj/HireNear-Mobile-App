...
export default function InvoiceDetailsScreen({ route, navigation }) {
        ...
        try {
            ...
            if (Platform.OS === 'android') {
                try {
                    ...
                    Alert.alert("Error", "Could not save the PDF file.");
                }
            } else {
    ...
    let badgeColor = 'bg-amber-100';
    let badgeText = 'Payment Pending';
    let badgeTextColor = 'text-amber-700';
    ...
    return (
        <SafeAreaView className="flex-1 bg-[#F8F9FB]">
            ...
            <ScrollView className="flex-1 p-5" showsVerticalScrollIndicator={false}>
                <View className={`self-start px-4 py-2 rounded-full mb-4 ${badgeColor}`}>
                    <Text className={`font-bold text-xs uppercase ${badgeTextColor}`}>
                        {badgeText}
                    </Text>
                </View>
                ...
                <View className="border border-slate-200 rounded-xl overflow-hidden bg-white mb-6 shadow-sm">
                    <View className="flex-row bg-slate-100 p-4 border-b border-slate-200">
                        ...
                        <Text className="w-28 font-extrabold text-slate-700 text-right">Cost</Text>
                    </View>
                    {invoice.items.map((item, index) => (
                        <View key={index} className="flex-row p-4 border-b border-slate-50">
                            <Text className="flex-1 text-slate-600 font-medium">{item.description}</Text>
                            <Text className="w-28 text-slate-900 text-right font-bold">LKR {item.amount}</Text>
                        </View>
                    ))}
                ...
                {role === 'worker' && invoice.status === 'verifying' && invoice.paymentSlipUrl && (
                ...
                {invoice.status === 'paid' && invoice.paymentSlipUrl && (
                    <View className="mb-10">
                        ...
                        <TouchableOpacity onPress={() => setViewerVisible(true)} className="border border-emerald-100 p-2 rounded-2xl bg-white shadow-sm relative">
                            <Image
                                source={{ uri: invoice.paymentSlipUrl || fallbackImage }}
