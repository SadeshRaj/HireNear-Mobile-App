...
import {
    ...
    Image, Alert, Linking, RefreshControl
} from 'react-native';
    ...
    const renderBidItem = ({ item }) => (
        <View className="bg-white rounded-[32px] p-5 mb-4 shadow-sm border border-gray-100">
            ...
            <View className="flex-row items-center mb-4">
                {/* Profile Image - Clickable to view portfolio */}
                <TouchableOpacity
                    onPress={() => navigation.navigate('WorkerPortfolio', { workerId: item.workerId?._id })}
                ...
                <View className="ml-3 flex-1">
                    ...
                    <View className="flex-row items-center mt-1">
                        ...
                        <Text className="text-slate-500 text-xs ml-1">
                            {item.distance ? `${item.distance} km away` : 'Location N/A'}
                        </Text>
                    </View>
                </View>

                <View className="items-end">
                    <Text className="text-emerald-700 font-black text-lg">Rs.{item.price.toLocaleString()}</Text>
